...
class RootID():
    ...
    def root_detection(self, data):
        ...
        for i in range(d):
            for j in [x for x in range(d) if x!=i]:   
                ...
                X_train, X_test, y_train, y_test = train_test_split(data[:,i].reshape(-1,1), 
                                                                    data[:,j].reshape(-1,1),
                                                                    test_size=0.1, 
                                                                    random_state=0)
    ...
    def DAG(self, n = 1000):
         ...
         data = np.column_stack(self.standardize_vectors(x,y,z,w))
         return data

    def simple_simulation(self, n = 1000, iter = 10):
        #Synthetic Empirical Evaluation
        count = 0
        start_time = time.time()
        for i in range(iter):
            data = self.DAG(n=n)
            roots = self.root_detection(data)
            if roots == [0,1]:
                count+=1
